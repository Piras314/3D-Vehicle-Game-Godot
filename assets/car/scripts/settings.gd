...
func SavePreset():
	...
	if (dir.open("user://assets/car") != OK):
		print("Preset directory doesn't exist")
		dir.make_dir("user://assets/car")
		print("..created a preset directory")
	preset_file.open("user://assets/car" + presetName.text + ".json", File.WRITE)
	...
	if (dir.open("user://assets/car")) == OK:
		...
		while (file_name != ""):
			if (file_name == PresetList.get_item_text(PresetList.get_selected_items()[0]) + ".json"):
				...
				dir.remove("user://assets/car" + PresetList.get_item_text(PresetList.get_selected_items()[0]) + ".json")
...
func LoadPreset():
	...
	if (preset_file.open("user://assets/car" + PresetList.get_item_text(PresetList.get_selected_items()[0]) + ".json", File.READ) != OK):
		print("Preset doesn't exist")
		return
	preset_file.open("user://assets/car" + PresetList.get_item_text(PresetList.get_selected_items()[0]) + ".json", File.READ)
